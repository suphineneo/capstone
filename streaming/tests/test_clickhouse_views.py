...
def test_valid_code_name(connect_to_clickhouse,coin_code_name_df):
    ...
    clickhouse_data = client.query_df('SELECT distinct code, name FROM coins_current_full order by code')
    clickhouse_data = pd.DataFrame(clickhouse_data, dtype="str")
    
    pd.testing.assert_frame_equal(left=clickhouse_data, right=coin_code_name_df, check_exact=True)
...
def test_ohlc_logic(connect_to_clickhouse):
    client = connect_to_clickhouse
    clickhouse_data = client.query_df('SELECT distinct(less(high - low, 0)) as high_low FROM v_ohlc_by_day')
    clickhouse_data['high_low'] = clickhouse_data['high_low'].astype("int64")

    expected_value = [{"high_low": 0}]  #high must be >= low in ohlc so the expected value of less(high-low) is 0
    expected_value = pd.DataFrame(expected_value, dtype="int64")

    pd.testing.assert_frame_equal(left=clickhouse_data, right=expected_value, check_exact=True)
